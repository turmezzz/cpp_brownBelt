...
private:
	...
	struct Segment
	{
		unordered_map <K, V, Hash> map_d;
		mutable mutex mutex_d;
	};

	mutable vector <Segment> data_d;
	size_t bucket_count_d;
	Hash hasher;
...
public:
	...
	explicit ConcurrentMap(size_t bucket_count) : bucket_count_d(bucket_count), data_d(bucket_count)
	{

	}

	WriteAccess operator[](const K& keyK)
	{
		auto key = hasher(keyK);
		auto segment_index = (key < 0 ? -key : key) % bucket_count_d;
		unique_lock <mutex> guard = unique_lock(data_d[segment_index].mutex_d);
		if (data_d[segment_index].map_d.find(keyK) == data_d[segment_index].map_d.end())
		{
			data_d[segment_index].map_d[keyK] = V();
		}
		return {data_d[segment_index].map_d[keyK], move(guard)};
	}

	ReadAccess At(const K& keyK) const
	{
		auto key = hasher(keyK);
		auto segment_index = (key < 0 ? -key : key) % bucket_count_d;
		unique_lock <mutex> guard = unique_lock(data_d[segment_index].mutex_d);
		return {data_d[segment_index].map_d.at(keyK), move(guard)};
	}

	bool Has(const K& keyK) const
	{
		auto key = hasher(keyK);
		auto segment_index = (key < 0 ? -key : key) % bucket_count_d;
		unique_lock <mutex> guard = unique_lock(data_d[segment_index].mutex_d);
		return data_d[segment_index].map_d.find(keyK) != data_d[segment_index].map_d.end();
	}

	MapType BuildOrdinaryMap() const
	{
		MapType ret;
		for (auto& segment : data_d)
		{
			unique_lock <mutex> guard(segment.mutex_d);
			for (auto pair : segment.map_d)
			{
				ret.insert(move(pair));
			}
		}
		return ret;
	}
