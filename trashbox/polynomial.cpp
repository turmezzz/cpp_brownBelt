...
struct Ref
{
	Ref& operator=(const T& coeff)
	{
		if (coeff != 0 and d_degre + 1 > d_coeffs.size())
		{
			d_coeffs.resize(d_degre + 1);
		}
		else if (coeff == 0 and d_degre + 1 == d_coeffs.size())
		{
			int i;
			for (i = d_coeffs.size() - 2; i >= 0; --i)
			{
				if (d_coeffs[i] != 0) break;
			}
			d_coeffs.resize(i + 1);
		}
		d_coeffs[d_degre] = coeff;
		return *this;
	}
...
template <typename T>
bool operator==(const Ref <T>& a, const T& value)
{
	if (a.d_degre + 1 > a.d_coeffs.size())
	{
		return 0 == value;
	}
	return a.d_coeffs[a.d_degre] == value;
}

template <typename T>
std::ostream& operator<<(std::ostream& out, const Ref<T>& ref)
{
	out << ref.d_coeffs[ref.d_degre];
	return out;
}
...
void TestDegreeDecrease()
{
	Polynomial <int> poly(std::initializer_list <int>{1, 2, 3, 4, 5});
...
int main()
{
	...
	RUN_TEST(tr, TestDegreeDecrease);
